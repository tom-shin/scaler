...
static const u32 sc_version_table[][2] = {
	{ 0x06000101, SCALER_VERSION(6, 0, 1) }, /* SC_POLY */
...
#if defined(CONFIG_EXYNOS_BTS) || defined(CONFIG_EXYNOS_BTS_MODULE)
#define sc_bts_get_bwindex(s)		bts_get_bwindex(s)
#define sc_bts_update_bw(id, bw)	bts_update_bw(bts_id, bw);
#else
#define sc_bts_get_bwindex(s)		-1
#define sc_bts_update_bw(id, bw)	do { } while (0)
#endif
...
static int sc_devm_iommu_direct_map(struct device *dev, u32 pa, size_t size)
{
	...
	#if 1	//fix parasoft generate unit tests.
	ret = iommu_map(domain, pa, pa, size, 0, GFP_KERNEL);
	#else
...
static int sc_probe(struct platform_device *pdev)
{
	...
	sc->bts_id = sc_bts_get_bwindex("MSCL");
