					...
					bool dyn_flag, bool st_flag)
{
	...
	if (freq_table) {
		...
		for (idx = 0; idx < table_cnt; idx++) {
			fv_table[idx].freq = freq_table[idx];
		}
		if (cal_dfs_get_freq_volt_table(cal_id,
					fv_table, table_cnt)) {
		...
		for (idx = 0; idx < table_cnt; idx++) {
		table[idx].freq = fv_table[idx].freq;
		table[idx].volt = fv_table[idx].volt;
		...
		if (!cal_ftable || !cal_vtable) {
			pr_err("mifpro: failed to alloc cal-table(cal-cnt=%d)\n",
					cal_table_cnt);
			kfree(table);
			return NULL;
		}
